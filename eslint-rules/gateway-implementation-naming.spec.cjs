...
ruleTester.run('gateway-implementation-naming', rule, {
  valid: [
    ...
    // Named export via class
    {
      ...
      filename: '/project/infra/notification-gateway/expo.notification.gateway.ts',
    },
    // Multi-word prefix and domain
    {
      code: `export class InMemoryBlockSessionGateway {}`,
      filename: '/project/infra/block-session-gateway/in-memory.block-session.gateway.ts',
    },
    ...
    {
      code: `export class Wrong {}`,
      filename: '/project/node_modules/package/infra/firebase.auth.gateway.ts',
    },
  ],
