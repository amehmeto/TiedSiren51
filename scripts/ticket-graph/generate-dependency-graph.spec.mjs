...
describe('parseDependencyRef', () => {
  ...
  describe('local refs', () => {
    it('parses bare number as local ref', () => {
      const result = parseDependencyRef('123', 'TiedSiren')
      expect(result).toEqual({ repo: 'TiedSiren', number: 123 })
    })

    it('parses #-prefixed number as local ref', () => {
      const result = parseDependencyRef('#456', 'TiedSiren')
      expect(result).toEqual({ repo: 'TiedSiren', number: 456 })
    })
  ...
  describe('invalid inputs', () => {
    ...
    it('returns null for unknown repo abbreviation', () => {
      const result = parseDependencyRef('UNKNOWN#123', 'TiedSiren')
    ...
    it('returns null for non-numeric issue number', () => {
      const nonNumericResult = parseDependencyRef('#abc', 'TiedSiren')
      expect(nonNumericResult).toBeNull()
    })

    it('returns null for malformed input', () => {
      const malformedResult = parseDependencyRef('not-valid', 'TiedSiren')
...
describe('depRefKey', () => {
  it('creates composite key from repo and number', () => {
    const key = depRefKey({ repo: 'TiedSiren', number: 123 })
    expect(key).toBe('TiedSiren#123')
  })
...
describe('formatDepRef', () => {
  it('formats local ref as #N when repo matches context', () => {
    const label = formatDepRef({ repo: 'TiedSiren', number: 123 }, 'TiedSiren')
    expect(label).toBe('#123')
  })

  it('formats cross-repo ref with abbreviation when different from context', () => {
    const label = formatDepRef({ repo: 'tied-siren-blocking-overlay', number: 9 }, 'TiedSiren')
    expect(label).toBe('TSBO#9')
  })

  it('uses TS abbreviation for TiedSiren in cross-repo context', () => {
    const label = formatDepRef({ repo: 'TiedSiren', number: 100 }, 'tied-siren-blocking-overlay')
    expect(label).toBe('TS#100')
  })

  it('formats EAS repo with correct abbreviation', () => {
    const label = formatDepRef({ repo: 'expo-accessibility-service', number: 5 }, 'TiedSiren')
    expect(label).toBe('EAS#5')
  })

  it('formats EFS repo with correct abbreviation', () => {
    const label = formatDepRef({ repo: 'expo-foreground-service', number: 3 }, 'TiedSiren')
    expect(label).toBe('EFS#3')
  })

  it('formats ELIA repo with correct abbreviation', () => {
    const label = formatDepRef({ repo: 'expo-list-installed-apps', number: 1 }, 'TiedSiren')
  ...
  it('falls back to full repo name if no abbreviation defined', () => {
    ...
    const label = formatDepRef({ repo: 'unknown-repo', number: 1 }, 'TiedSiren')
